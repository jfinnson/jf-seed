angular.module('app.marketplace.templates', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('templates/marketplace/main/mainPage.html',
    '<div id=mainPage class=mainPage ng-controller=SystemController>Test8 <span ng-repeat="product in products" style=float:left;width:100%>{{product.name}}</span></div>');
  ...
  $templateCache.put('templates/shared/support/testing-state1.html',
    '<div>State1</div>');
  $templateCache.put('templates/shared/support/testing.html',
    '<div class=simpleDemo>Test page<div ui-view></div></div>');
}]);
