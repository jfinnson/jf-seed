angular.module('app.marketplace.templates', []).run(['$templateCache', function($templateCache) {
  $templateCache.put('templates/marketplace/main/mainPage.html',
    '<div id=mainPage class=mainPage ng-controller=SystemController>Test <span ng-repeat="product in products" style=float:left;width:100%>{{product.name}}</span></div>');
