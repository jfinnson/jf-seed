...
angular.module("app.marketplace.elements")
    /*
     * Service for accessing, maintaining, and updating element types and instances.
     * 
     */
    .service("elementSrv", [
            "apiUtils", "onlineUtils", "serverAPI",
            "accountModel", "productModel",
            "$rootScope", "$q", "$timeout",
            function(apiUtils, onlineUtils, serverAPI, 
                accountModel, productModel
                eleTreeManager, $rootScope, $q, $timeout) {
              ...
              var _create = function(eleSrv, options) {
                ...
                _setGenMethods(eleSrv, newEle); //Set generic methods for instance
                ...
                //Save UI vars
                var trackUI = _eleMaps[eleSrv.model_data.name][trackID]
                    && _eleMaps[eleSrv.model_data.name][trackID].UI ? _eleMaps[eleSrv.model_data.name][trackID].UI
                    : {};
                newEle.UI = trackUI;
              ...
              //Sets generic methods for instance that every instance should have.
              var _setGenMethods = function(eleSrv, eleIns) {
                ...
                eleIns.set = function(name, value) {
                  ...
                  if (eleSrv.fields[name].type==="object" || eleSrv.fields[name].type==="array") {
                    if (value){
                      angular.copy(value, eleIns[name]); //Copies values while maintaining reference links to eleIns[name]
                    }
                  }else {
                    eleIns[name] = value;
                  }
                ...
                if (!eleIns[name]) {
                  ...
                  if (eleSrv.fields[name].default_value) {
                    if (fieldType === "object" || fieldType === "array") {
                      eleIns[name] = $.extend(true, eleIns[name],
                          eleSrv.fields[name].default_value);
                    } 
                    //May not want to store date object in the field
//                    else if (fieldType === "date" || fieldType === "duration") {
...
//                    } 
              ...
              var _updateBdata = function(eleIns, eleType, options) {
                ...
                //Get all fields that should be submitted to the api (have api.submit==true)
                for ( var fieldName in eleIns) {
                  currFieldParams = eleSrv.fields[fieldName];
                  if (currFieldParams && currFieldParams.api
                      && currFieldParams.api.submit
              ...
              /*
               * General method for retrieving _elements from db and local storage
               * 
              ...
              var _get = function(eleType, id, options) {
                ...
                //If single instance is requested, there is no force, and it exists in memory, then provide the instance in memory.
                if(id && (!options || !options.forceAPI) && _eleMaps[eleType][id]){
                  return $q(function(resolve,reject){
                    resolve(_eleMaps[eleType][id]);
                  });
                }else if(id){
                  //If id is provided, and force is applied or does not exist in memory, then setup options to do an api call.
                  if(!options){
                    options = {};
                  }
                ...
                return serverAPI.doAPICall(path, options).then(function(response){
                    ...
                    for (var i in eles) {
                      ...
                      currEleIns = _create(eleSrv, currEleIns); //Create ele while keeping untouched b-data
               ...
               */
              var _submit = function(eleType, options) {
                ...
                var serverSubmitEle = function(path, currOptions, currEleIns) {
                  return serverAPI.doAPICall(path, currOptions).then(function(response){
                    ...
                    var currReEleIns = updateInstance(currEleIns, newEleData);
                ...
                for ( var i in postEles) {
                  ...
                  var bDataUpdate = _updateBdata(currEleIns, eleTypeName, options);
              ...
              //Validate instance's fields against their parameters (correct type etc.)
              //TODO
              var _validateFields = function(eleType, currEle, options) {
                if (!options)
                  options = {};
                if (!eleType || !currEle) {
                ...
                var valResult = null;
                var valResutls = [];
                for (var currField in fieldParams) {
//                  valResult = validateSrv.validateField(currEle[currField], fieldParams[currField]); TODO
                  valResult = {"pass" : true};
              ...
              var _updateInstance = function(ele, newData) {
                ele.b_data = $.extend(true, ele.b_data || {}, newData);
                ele = $.extend(true, ele || {}, newData);
                return ele;
              };
              //Remove instance from global map. Also remove from backend if apiRemove==true
              ...
              var _initModelMethods = function() {
                for ( var i in _eleNameMap) {
                  //TODO would put generic fns or params here. (like local store methods)
                }
              };
