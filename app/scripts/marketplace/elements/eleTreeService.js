...
//Treated like an ele due to its model-like nature. //TODO reassess either this manager or all others.
...
.service("eleTreeManager", ["serverAPI","apiUtils", function(serverAPI, apiUtils) {
	...
	this.eleTree = {
		...
		"createTree" : function(eles){
			...
			for(var key in eles){
				...
				if(parentKey!==null){
					...
					if(parentKey && eles[parentKey])eles[parentKey].f_data.hasChildren = true;
			...
			//Need to do ordering here to allow for efficient and complete ordering
			...
			for(key in this.f_data.treeMap){
				currChildMap = $.extend(true, {}, this.f_data.treeMap[key]); //Need to extend to allow for removal
				currChildArrayMap = this.f_data.treeArrayMap[key]
				currChildMapIndex = 0;
				currChildMapCount = 0;
				originalChildMapLength = Object.keys(currChildMap).length;
				
				//Do while elements exist or has reached an error. Should never take n*n time.
				while(Object.keys(currChildMap).length!=0 && currChildMapCount<(originalChildMapLength*originalChildMapLength)){
					...
					currEle = this.f_data.treeMap[key][currKey]; // Important to use "this.f_data.treeMap[key]" to allow for data linking
					inserted = false;
					
					//Specify key if missing 
					if(!currEle.f_data.key) currEle.f_data.key = currKey;
					...
					if((treeIns.f_data.rootType && currEle.f_data.type!=treeIns.f_data.rootType) && key=="0" ){
						...
						if(!treeIns.f_data.exceptionType || !currEle.f_data[treeIns.f_data.exceptionType]){
							if(!this.f_data.treeArrayMap['misc'])this.f_data.treeArrayMap['misc'] = [];
							this.f_data.treeArrayMap['misc'].push(currEle);
						} 
						inserted = true; 
					}else if(currEle.f_data.previousKey){			//Insert according to order. Dictated by previousKey field 
						...
						for(var currChildIndex in currChildArrayMap){
							...
							if(currChild.f_data.key==currEle.f_data.previousKey){
								...
								currChildArrayMap.splice((parseInt(currChildIndex)+1), 0, currEle);
								inserted = true;
								break;
							}
						}  
					...
					if(inserted){
						//Case 4. Not inserted. Likely due to previous ele not being inserted yet.
					...
					currChildMapIndex++;
					currChildMapCount++;
					if(currChildMapIndex>=Object.keys(currChildMap).length)currChildMapIndex=0;
				}
				
				//Case 5 eles with errors or missing parents or missing previous. Put all into misc array
		...
		"createKey" : function(newEle,eles){ 
			var increment = 0;
			while(!newEle.f_data.key || 
					(eles[newEle.f_data.key] && eles[newEle.f_data.key]!=newEle)){
		...
		"addEle" : function(newEle, baseEle){
			if(!newEle || !newEle.f_data.key) return null;
			...
			if(!this.f_data.treeMap[parentKey]) this.f_data.treeMap[parentKey] = {};
			if(!this.f_data.treeArrayMap[parentKey]) this.f_data.treeArrayMap[parentKey] = [];
		...
		"updateEle" : function(currEleKey, newChildArray, newParent, end, oldChildArray, start){
			...
			var newParentKey = 0;
			if(newParent) newParentKey = newParent.f_data.key;
			var newParentLuid = null;
			if(newParent) newParentLuid = newParent.f_data.luid;
	        ...
	        if(!newChildArray.length && newParent) newParent.f_data.hasChildren= true; 
	        newChildArray.splice(end, 0, oldChildArray.splice(start, 1)[0]);
	        if(!oldChildArray.length && oldChildArray!=newChildArray && oldParentKey) this.eles[oldParentKey].f_data.hasChildren= false;
		}
